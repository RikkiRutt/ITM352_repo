        ...
        function getCookieValue(cookieName) {
            ...
            for (let i = 0; i < cookies.length; i++) {
                ...
                if (cookiePair[0] === cookieName) {
                    return cookiePair[1]; //return value if name matches
                }
            }
    ...
    if (typeof user_reg_data[username_entered] != 'undefined') {
        ...
        if (password_entered == user_reg_data[username_entered].password) {
            ...
            if(!userSession.lastLogin) {
                userSession.lastLogin = "FirstVisit!"
            } else {
    ...
    login_form = `
        <script>
            ...
            window.onload = function() {
                if (params.has('error')) {
                    login_form['username'].value = params.get('username');
                    login_form['email'].value = params.get('email');
                    login_form['name'].value = params.get('name');
                }
            }
        </script>
